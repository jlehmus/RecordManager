...
class Ead3 extends Ead
{
    ...
    */
    public function __construct($data)
    {
        ...
        // dev debug vv
//	$this->archiveId = strlen($data) . "_" . $this->archiveId;
...
// xpath
		...
		// shouldent happen:
                if ($unitId == '') {
                    $unitId = urlencode($this->archiveId . '_POS' 
                        . $this->currentPos); 
                }

            } else {
//                $unitId = $this->archiveId . '_pos' . $this->currentPos;
            ...
            if ($parentDid) {
                ...
                if (!$parentTitle) {	
                    $parentTitle = (string)$parentDid->unittitle->attributes()->label; 
                ...
                if ($prependParentTitleWithUnitId) {
                    $pid = implode('+',$parentDid->xpath('unitid[@label="Analoginen"]'));
